...
function runMigrations() {
    echo "--------------------------------------"
    echo "Running prisma migrations/generation"
    echo "--------------------------------------"
    if [ ! -d "$ROOT_DIR/libs/db/src/prisma/migrations" ]; then
        echo -n "Generating initial migration..."
        yarn prisma:migrate:initialize 2&> /dev/null
        echo " Done"
    fi
